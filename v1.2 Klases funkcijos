...
vector<studentas> failo_nuskaitymas3(const string& filename)
{
    ...
    try {
        if (file.is_open()) {
            ...
            string line;
            int numLines = count(istreambuf_iterator<char>(file), istreambuf_iterator<char>(), '\n');
            students.reserve(numLines);
            geri.reserve(numLines);
            ...
            while (getline(file, line))
            {
                ...
                if (0.4 * student.vidurkio_nuskaitymas() + 0.6 * student.egzamino_nuskaitymas() < 5)
                    geri.push_back(std::move(student));
                else
...
void testStudentas() {
    ...
    studentas s1;
    s1.vardo_nustatymas("Jonas");
    s1.pavardes_nustatymas("Maciulis");
    s1.egzamino_nustatymas(8);
    s1.pazymiu_nustatymas({5, 8, 9});

    studentas s2(s1);

    s2.vardo_nustatymas("Jadvyga");
    s2.pazymiu_nustatymas({5, 9, 5});
    ...
    cout << "Student 1: " << s1.vardo_nuskaitymas() << " " << s1.pavardes_nuskaitymas() << endl; 
    cout << "Student 2: " << s2.vardo_nuskaitymas() << " " << s2.pavardes_nuskaitymas() << endl; 
    cout << "Student 3: " << s3.vardo_nuskaitymas() << " " << s3.pavardes_nuskaitymas() << endl; 
    cout << "Student 4: " << s4.vardo_nuskaitymas() << " " << s4.pavardes_nuskaitymas() << endl; 

    cout << "2 studento pazymiai: ";
    for (int grade : s2.pazymiu_nuskaitymas()) {
        cout << grade << " ";
    }
    cout << endl;
    ...
    cout << "2 studento surikiuoti pazymiai: ";
    for (int grade : s2.pazymiu_nuskaitymas()) {
        cout << grade << " ";
    }
    cout << endl;
