...
void generuoti_faila_duomenis(int studentu_skaicius, int pazymiu_skaicius, const string& filename) {
    ...
    if (file.is_open()) {
        ...
        file.close();
        cout << "Failas su duomenimis sugeneruotas: " << filename << ". ";
    }
    else {
...
vector<studentas> failo_nuskaitymas(const string& filename)
{
    ...
    try {
        if (file.is_open()) {
            ...
            auto startPrinting = chrono::steady_clock::now();
            spausdinti_i_faila(geri, "angelai");
            spausdinti_i_faila(blogi, "demonai");
            auto endPrinting = chrono::steady_clock::now();
            auto durationPrinting = chrono::duration_cast<chrono::seconds>(endPrinting - startPrinting);
            cout << "Laikas vykdant spausdinima i faila: " << durationPrinting.count() << " s" << endl;
        }
        else {
            throw runtime_error("Nepavyko atidaryti failo: " + filename);
        }
